...
type pluginCopyContext struct {
	...
	skipRunDueToFailedCompile bool
}

var CMD_PLUGIN_COPY = cli.Command{
	Name:      "copy",
...
func (pctx *pluginCopyContext) startFileWatcher() {
...
func (pctx *pluginCopyContext) onFileChangeHandler(c *utility.WatchFileContext, eventPath string) {
	...
	if pctx.run {
		if !pctx.skipRunDueToFailedCompile {
	...
	if err != nil {
		...
		pctx.skipRunDueToFailedCompile = true
		return
	}
	...
	pctx.skipRunDueToFailedCompile = false
	if pctx.run {
...
func (pctx *pluginCopyContext) copyPluginFiles() error {
...
func (pctx *pluginCopyContext) runBinaryBuildCommand() error {
...
func (pctx *pluginCopyContext) runGoCMS() {
	fmt.Printf("Running GoCMS\n")
	go utility.StartGoCMS(pctx.destDir, pctx.devMode, pctx.goCMSDoneChan)
}

func (pctx *pluginCopyContext) getBinaryBuildCommand() error {
...
func (pctx *pluginCopyContext) goGenerate() error {
	// run go generate
...
func buildContextFromFlags(c *cli.Context) (*pluginCopyContext, error) {
	...
	pctx.ignorePath = append(pctx.ignorePath, []string{"vendor", ".git", "docs", ".idea", "___*", "node_modules", "content/src"}...)
...
func (pctx *pluginCopyContext) load_interface_for_plugin() {
...
func (pctx *pluginCopyContext) loadFileOrUrl(path string) {
